...
"""
🌐 BROWSER LAUNCHER v10.3 — ИСПРАВЛЕНЫ ТАЙМАУТЫ И SHUTDOWN ERRORS
✅ Первая навигация до загрузки stealth скриптов
✅ Cookies работают правильно
✅ Таймауты исправлены!
✅ None checks в close() методе
"""
...
class BrowserLauncher:
    ...
    async def launch(self, acc_id: str) -> Optional[Page]:
        ...
        try:
            ...
            # ─────────────────────────────────────────────────────
            # 8. ✅ ПЕРВАЯ НАВИГАЦИЯ НА ПУСТУЮ СТРАНИЦУ
            # ─────────────────────────────────────────────────────
            # 
            # ВАЖНО! Первая навигация на about:blank чтобы инициализировать
            # обработку событий и убедиться что Playwright готов
            # 
            try:
    ...
    async def close(self, acc_id: str) -> None:
        """✅ ИСПРАВЛЕННЫЙ: Закрыть браузер с проверками на None"""
        
        try:
            await self.set_browser_status(acc_id, "🛑 ЗАКРЫВАЕТСЯ")
            
            # ✅ Сохраняем cookies ТОЛЬКО если контекст существует и не None
            ...
            # ✅ Закрываем контекст с проверкой на None
