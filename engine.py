...
"""
🔥🏍️ WARMUP ENGINE 2031 — MOTO GOD MODE ULTRA — ПОЛНЫЙ КОД БЕЗ СОКРАЩЕНИЙ
═══════════════════════════════════════════════════════════════════════════════

✅ ТОЛЬКО РЕАЛЬНЫЕ ДЕЙСТВИЯ
✅ ЧЕСТНОЕ ЛОГИРОВАНИЕ
✅ 7–8 ФАЗ (90–150 МИН)
✅ ГЛАВНЫЙ УКЛОН — МОТОТЕХНИКА (ПИТБАЙКИ 40–60K)
✅ ПОЛНЫЙ DEEP VIEW КАРТОЧЕК
✅ СИСТЕМА СОСТОЯНИЙ ЧЕЛОВЕКА
✅ GRACEFUL SHUTDOWN
✅ ПОЛНОЕ СОХРАНЕНИЕ COOKIES + STORAGE
✅ АВТОМАТИЧЕСКИЙ ЗАПУСК ALIVE MODE ПОСЛЕ ПРОГРЕВА
✅ ДЕТАЛЬНОЕ ЛОГИРОВАНИЕ
✅ TELEGRAM УВЕДОМЛЕНИЯ
✅ BEHAVIOR PROFILES ДЛЯ СОВМЕСТИМОСТИ
✅ ОПТИМИЗИРОВАН ДЛЯ МЕДЛЕННЫХ ПРОКСИ (3x МНОЖИТЕЛЬ)
✅ БОЛЬШИЕ ЗАДЕРЖКИ ДЛЯ ИЗБЕЖАНИЯ БЛОКИРОВОК
✅ RETRY ЛОГИКА ДЛЯ ПОИСКОВ
...
class WarmupEngine:
    ...
    async def run_full_warmup(
        ...
        """🔥 ЗАПУСТИТЬ ПОЛНЫЙ ПРОГРЕВ (7 ФАЗ)"""
        
        start_time = datetime.now()
        self.human_state.session_start_time = start_time
        ...
        try:
            ...
            self.logger.success(acc_id, f"Warmup complete: {self.phases_completed}/7 phases, {self.total_warmup_duration/60:.1f} min")
            ...
            self.total_warmup_duration = (datetime.now() - start_time).total_seconds()
    ...
    async def _run_phase(
        ...
        try:
            ...
            self.current_phase_duration = phase_duration
            
            if success:
                self.phases_completed += 1
                print(f"\n  {Fore.GREEN}✅ Фаза {phase_num} завершена ({phase_duration:.1f} мин){Style.RESET_ALL}\n")
                self.logger.success(acc_id, f"Phase {phase_num} complete: {phase_duration:.1f} min")
            else:
                print(f"\n  {Fore.YELLOW}⚠️ Фаза {phase_num} завершена с ошибками ({phase_duration:.1f} мин){Style.RESET_ALL}\n")
                self.logger.warning(acc_id, f"Phase {phase_num} completed with errors: {phase_duration:.1f} min")
    ...
    async def _safe_search(self, page: Page, acc_id: str, navigator: AvitoNavigator, query: str, attempts: int = 3) -> bool:
        """🔍 БЕЗОПАСНЫЙ ПОИСК С RETRY"""
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                success = await navigator.search(page, query)
    ...
    async def _phase_1_basic_orientation(
        ...
        try:
            ...
            for i in range(random.randint(2, 3)):
                ...
                try:
                    success_click = await navigator.click_listing(page, i)
                    if success_click:
                        print(f"     {Fore.GREEN}✅ Карточка открыта{Style.RESET_ALL}")
                        
                        await self._view_item_realistic(page, acc_id, duration=random.randint(15, 25))
                        
                        try:
                            await navigator.go_back(page)
                        except:
                            pass
        ...
        try:
            ...
            for search_idx, search_query in enumerate(pit_bike_searches, 1):
                ...
                for item_idx in range(items_to_view):
                    ...
                    try:
                        success_click = await navigator.click_listing(page, item_idx)
                        if success_click:
                            ...
                            await self._view_item_realistic(page, acc_id, duration=duration)
                            
                            if random.random() < 0.35:
                                print(f"     {Fore.CYAN}🤔 Раздумье перед избранным...{Style.RESET_ALL}")
                                await self._human_pause(acc_id, "раздумье перед избранным", 20, 40)
                                
                                success_fav = await navigator.add_to_favorites(page)
                                if success_fav:
                    ...
                    try:
                        ...
                        if success_click:
                            ...
                            for _ in range(random.randint(5, 8)):
                                await page.evaluate(f"window.scrollBy(0, {random.randint(150, 250)})")
                            ...
                            if random.random() < 0.4:
                                ...
                                success_seller = await navigator.view_seller_profile(page)
                                if success_seller:
                            ...
                            print(f"     {Fore.GREEN}✅ Вернулось{Style.RESET_ALL}")
                            self.human_state.items_viewed += 1
                            await self._human_pause(acc_id, "между анализами", 10, 20)
                        else:
                            print(f"     {Fore.YELLOW}⚠️ Не удалось открыть{Style.RESET_ALL}")
                    except Exception as e:
                        print(f"     {Fore.YELLOW}⚠️ Ошибка: {e}{Style.RESET_ALL}")
                        ...
                        try:
                            ...
                            if success_click:
                                ...
                                try:
                                    ...
                                    pass
                                self.human_state.items_viewed += 1
                                await self._human_pause(acc_id, "между просмотрами", 8, 15)
                        except Exception as e:
                            print(f"     {Fore.YELLOW}⚠️ Ошибка: {e}{Style.RESET_ALL}")
                ...
                for item_idx in range(random.randint(4, 6)):
                    ...
                    try:
                        ...
                        if success_click:
                            ...
                            print(f"     {Fore.CYAN}Анализирую для избранного (40–60s)...{Style.RESET_ALL}")
                            await self._view_item_realistic(page, acc_id, duration=random.randint(40, 60))
                            ...
                            print(f"     {Fore.GREEN}✅ Вернулось{Style.RESET_ALL}")
                            self.human_state.items_viewed += 1
                            await self._human_pause(acc_id, "между избранными", 10, 20)
                        else:
                            print(f"     {Fore.YELLOW}⚠️ Не удалось открыть{Style.RESET_ALL}")
                    except Exception as e:
                        print(f"     {Fore.YELLOW}⚠️ Ошибка: {e}{Style.RESET_ALL}")
                    ...
                    try:
                        ...
                        if success_click:
                            ...
                            if success_seller:
                                ...
                                print(f"     {Fore.CYAN}Изучаю рейтинг и отзывы (20–40s)...{Style.RESET_ALL}")
                                for _ in range(random.randint(3, 5)):
                                    await page.evaluate(f"window.scrollBy(0, {random.randint(200, 300)})")
                                    await asyncio.sleep(random.uniform(2.0, 4.0))
                            ...
                            print(f"     {Fore.GREEN}✅ Вернулось{Style.RESET_ALL}")
                            self.human_state.items_viewed += 1
                            await self._human_pause(acc_id, "между продавцами", 12, 20)
                        else:
    ...
    async def _view_item_realistic(
        self,
        page: Page,
        acc_id: str,
        duration: int = 15,
    ) -> None:
        """�� РЕАЛИСТИЧНЫЙ ПРОСМОТР КАРТОЧКИ (ОПТИМИЗИРОВАН ДЛЯ ПРОКСИ)"""
        ...
        try:
            ...
            while datetime.now().timestamp() < end_time:
                action = random.choice(["scroll", "wait", "scroll_up"])
            ...
            self.logger.warning(acc_id, f"View item error: {e}")
            ...
            while self.running and not night_mode.is_night_time():
                ...
                try:
                    if action == "search":
                        ...
                        query = random.choice(["питбайк", "питбайк kayo", "квадроциклы"])
                        await navigator.search(page, query)
                        print(f"     ✅ Поиск: '{query}'")
                    
                    elif action == "view":
                        print(f"     📄 Просмотр...")
                        idx = random.randint(0, 3)
                        success = await navigator.click_listing(page, idx)
                        if success:
                        ...
                        if success:
                            await asyncio.sleep(random.uniform(8, 15))
                            await navigator.view_seller_profile(page)
                            try:
                                await navigator.go_back(page)
                            except:
                                pass
                            print(f"     ✅ Профиль просмотрен")
                        else:
                            print(f"     ⚠️ Не удалось открыть")
                ...
                pause_min = random.randint(60, 180)
...
class BehaviorProfile:
    """🎭 Профиль поведения по времени суток"""
    
    name: str
    description: str
    time_range: Tuple[int, int] = (0, 24)
    scroll_patterns: List[str] = field(default_factory=list)
    search_queries: List[str] = field(default_factory=list)
    action_probabilities: Dict[str, float] = field(default_factory=dict)
    base_tiredness_factor: float = 0.1
    base_interest_level: float = 0.7
    session_duration_min: int = 10
    session_duration_max: int = 30
    items_to_view_min: int = 3
    items_to_view_max: int = 8
    
    def matches_time(self, dt: Optional[datetime] = None) -> bool:
        if dt is None:
            dt = datetime.now()
        hour = dt.hour
        return self.time_range[0] <= hour < self.time_range[1]


class BehaviorProfileSelector:
    """🎭 Селектор профилей поведения"""
    
    @staticmethod
    def get_all_profiles() -> List[BehaviorProfile]:
        return [
            BehaviorProfile(name="Morning", description="Morning profile", time_range=(6, 12)),
            BehaviorProfile(name="Afternoon", description="Afternoon profile", time_range=(12, 18)),
            BehaviorProfile(name="Evening", description="Evening profile", time_range=(18, 23)),
            BehaviorProfile(name="Night", description="Night profile", time_range=(23, 6)),
        ]
    
    @staticmethod
    def get_profile(dt: Optional[datetime] = None) -> BehaviorProfile:
        if dt is None:
            dt = datetime.now()
        
        for profile in BehaviorProfileSelector.get_all_profiles():
            if profile.matches_time(dt):
                return profile
        
        return BehaviorProfileSelector.get_all_profiles()[-1]